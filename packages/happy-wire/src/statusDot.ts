 ...
 *
 * WHY IT LIVES ON THE WIRE NOW (DROVE-247). It was written in the app, because
 * the app was the only thing that drew a dot. The terminal is the third
 * renderer and it cannot be served the same way: the phone resolves the state
 * from a snapshot the CLI published minutes ago over the internet, and a dot in
 * Clay's tmux status line has to be right about the pane in front of him
 * whether or not the phone is even awake. The only process that can see a
 * session's facts the instant they change is the happy-cli driving that pane.
 *
 * So happy-cli resolves and publishes, and for that it needs this table — and
 * `@slopus/happy-wire` is the one package both it and the app already depend
 * on. Moving the file is what keeps the count at ONE table. `statusDotState.ts`
 * in the app is now a re-export, so every existing import and both existing
 * specs still read the same values from the same lines.
 */
 ...
 *
 * Lives here rather than in the app's `utils/liveStatus` (where it was
 * written) because `DISCONNECT_RECENT_MS` below is defined as this number and
 * the two must not be able to drift across a package boundary. The app
 * re-exports it from its old home, so nothing that imported it has moved.
 */
