 ...
 *
 * Also undefined when the newest copy is ONE FILE reached under several
 * names. Since DROVE-40 every account's projects/ is a symlink into
 * ~/.claude-shared/projects, so the same inode answers for all of them and
 * the mtime tie used to fall to whichever account the registry listed first —
 * always main — which is a guess dressed as a measurement (DROVE-21). A shared
 * store names no account; the caller keeps what it knew.
 */
export function accountByNewestTranscript(
    ...
    for (const a of readAccounts()) {
        ...
        if (mtime > bestMtime) {
            ...
            best = a
            bestShared = false
        } else if (mtime === bestMtime && real === bestReal) {
            bestShared = true
        }
    }
    return bestShared ? undefined : best
}
