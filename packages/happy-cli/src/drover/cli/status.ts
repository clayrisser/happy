...
export async function statusReport(options: StatusOptions = {}): Promise<string[]> {
    ...
    const logsDir = join(de.happyHome, 'logs');
    renderPush(sink, pushVerdicts(pushLogs(pushCandidates(logsDir, window), window)), de.happyHome, de.forkDir);
    ...
    renderDaemon(sink, probe, de.happyHome);
    renderBuild(sink, de.forkDir, de.stateDir, probe.now());
    renderSleep(sink, probe.pmset());
    renderServices(sink, probe, home, de.serverMode);

    out.push('');
    out.push(`mode      ${de.serverMode} (${de.serverMode === 'relay' ? de.relayUrl : 'official Happy server'})`);
    out.push(`happy     ${de.happyHome}`);
    return out;
}
