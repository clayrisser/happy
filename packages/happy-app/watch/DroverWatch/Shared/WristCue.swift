...
enum WristCue: String, Codable, CaseIterable {
    /// Claude wants Clay to DO something (DROVE-53).
    ///
    /// The raw value is the WIRE kind, and the wire kind is `todo`: that is
    /// what droverGates stamps and what DroverGate.Kind decodes. This cue is
    /// selected by `WristCue(rawValue: gate.kind)`, so spelling it anything
    /// else silently drops a needs-you request onto the permission cue and
    /// costs it the pattern it is supposed to be told apart by.
    case needsYou = "todo"
