...
export const ru: TranslationStructure = {
    ...
    errors: {
        ...
        voiceNotAllowed: 'Голосовой сервер отказал в запуске режима босса. Нужны голосовые минуты или подписка на этом сервере, а эта сборка не может показать экран оплаты.',
    ...
    sessionInfo: {
        ...
        remoteControlTitle: 'Удалённое управление',
        remoteControl: 'Удалённое управление Claude Code',
        remoteControlOn: 'Включено — сессия доступна с телефона и через claude.ai/code',
        remoteControlOff: 'Выключено — ответы остаются в терминале',
        remoteControlPending: 'Ждём, когда освободится приглашение…',
        remoteControlUnknown: 'Терминал ещё не ответил',
        remoteControlFooter: 'Читает, что панель делает на самом деле, поэтому /remote-control, набранный в терминале, тоже виден здесь.',
        remoteControlUnknownFooter: 'Переключать нечего, пока сессия не сообщит состояние удалённого управления.',
        remoteControlTurnOn: 'Включить',
        remoteControlAtRiskTitle: 'Замолчали после смены аккаунта',
        remoteControlAtRiskFooter: 'Claude Code привязывает удалённое управление к одному аккаунту на машину, поэтому переезд этой сессии оборвал их.',
    ...
    agentInput: {
        streamTalk: {
            ...
            onHint: 'Ответы читаются вслух. Коснитесь предложения, чтобы читать с него',
        ...
        dictate: {
            ...
            settingsSubtitle: 'Кнопка микрофона в поле ввода: удерживайте, чтобы говорить, коснитесь, чтобы оставить включённым. Распознаётся на этом устройстве.',
            listening: 'Слушаю…',
        ...
        channels: {
            ...
            phoneHaptics: 'Вибрация телефона',
            phoneHapticsSubtitle: 'Разрешить этому телефону вибрировать: приходящие запросы и мелкие отклики под пальцем. Выключено — часы всё равно вибрируют, а телефон молчит.',
            phoneHapticsTitle: 'ЭТОТ ТЕЛЕФОН',
            phoneHapticsFooter: 'Запястье — та поверхность, которая должна вас тронуть. Телефон вы смотрите и слушаете, и он лежит в кармане, пока читается ответ, поэтому по умолчанию он молчит. Один переключатель на оба вида: оповещения сессии и отклик на касание. Включение ничего не меняет в часах, выключение ничего у часов не отнимает.',
        },
        codexPermissionMode: {
        ...
        usagePopup: {
            ...
            familyWeek: ({ family }: { family: string }) => `Неделя ${family}`,
            left: ({ percent }: { percent: number }) => `Осталось ${percent}%`,
    ...
    toolGroup: {
        ...
        ranShellCommands: ({ count }: { count: number }) => `${plural({ count, one: 'Выполнена', few: 'Выполнено', many: 'Выполнено' })} ${count} ${plural({ count, one: 'команда оболочки', few: 'команды оболочки', many: 'команд оболочки' })}`,
    ...
    tools: {
        ...
        taskView: {
            ...
            showAll: ({ count }: { count: number }) => `Показать все шаги (${count})`,
            showLess: 'Свернуть',
        },
        multiEdit: {
    ...
    settingsVoice: {
        ...
        speakOn: {
            title: 'Озвучивать ответы на',
            footer: 'Какое устройство читает ответ вслух. Авто следует за наушниками: телефон говорит, когда они подключены к нему, часы, когда они подключены к часам, иначе телефон. Часы вибрируют в начале ответа, кто бы ни говорил.',
            phone: 'Телефон',
            watch: 'Часы',
            auto: 'Авто',
            autoDetail: 'То, к которому подключены наушники',
        },
        speaking: {
            title: 'Голос чтения',
            footer: 'Какой установленный голос читает ответы вслух. Премиум и улучшенные голоса звучат по-человечески; компактный, стандартный, звучит как робот. Другие голоса загружаются в Настройки > Универсальный доступ > Устный контент > Голоса.',
            automatic: 'Автоматически',
            automaticSubtitle: ({ name }: { name: string }) => `Лучший установленный голос: ${name}`,
            automaticNone: 'Для этого языка нет установленных голосов',
            preview: 'Прослушать',
            previewSentence: 'Тесты проходят, два файла изменены, больше ничего не тронуто.',
            qualityPremium: 'Премиум',
            qualityEnhanced: 'Улучшенный',
            qualityDefault: 'Компактный',
            personal: 'Личный голос',
            deliveryTitle: 'Подача',
            deliveryFooter: 'Скорость и высота применяются к любому голосу. Законченный ответ читается до конца, каким бы длинным он ни был. Пока ответ ещё пишется, если непроизнесённого звука остаётся больше порога, голос говорит «пропускаю вперёд» и продолжает с последнего предложения; новый ответ всегда обрывает предыдущий.',
            rate: 'Скорость',
            catchUpRate: 'Скорость при отставании',
            pitch: 'Высота',
            backlog: 'Ускоряться при отставании на',
            jump: 'Перескакивать при отставании на',
            seconds: ({ seconds }: { seconds: number }) => `${seconds} с`,
            noNaturalVoice: ({ language }: { language: string }) => `Для ${language} не установлен улучшенный или премиум-голос, поэтому ответы читает компактный. В Настройках откройте Универсальный доступ > Устный контент > Голоса и загрузите голос.`,
            openSettings: 'Открыть Настройки',
        },
        cues: {
            title: 'Звуковые сигналы',
            footer: 'Звуки для работы без взгляда на экран. Медленный низкий удар, пока что-то выполняется, более частый яркий импульс, пока что-то ждёт вас, и тишина, когда ничего не происходит. Речь всегда важнее: сигнал никогда не звучит поверх произносимой фразы.',
            on: 'Звуковые сигналы',
            onSubtitle: 'Пульс и одиночные звуки.',
            heartbeat: 'Пульс',
            heartbeatSubtitle: 'Бьётся, пока идёт работа и ничего не произносится.',
            volume: 'Громкость',
            workingEvery: 'Пульс работы каждые',
            waitingEvery: 'Пульс ожидания каждые',
            titlesTitle: 'Произносимые заголовки',
            titlesFooter: 'Однострочный заголовок вызова инструмента, терминала или запускаемого агента, читаемый быстрее и выше ответа, чтобы не звучать как речь Claude. Серия вызовов ограничена; агенты называются всегда.',
            speakTitles: 'Читать заголовки вслух',
            agentTitles: 'Заголовки агентов',
            agentTitlesSubtitle: 'Никогда не пропускаются.',
            toolTitles: 'Заголовки инструментов и терминала',
            titlesPerRun: 'Заголовков на серию вызовов',
            tableTitle: 'Звуки',
            tableFooter: 'Нажмите строку, чтобы отключить её, или кнопку воспроизведения, чтобы услышать.',
            play: 'Воспроизвести',
        },
    },
