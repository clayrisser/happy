...
export const zhHans: TranslationStructure = {
    ...
    server: {
        ...
        changeServer: '更改服务器',
        continueWithServer: '同一把密钥就是同一个账户。会话不会跟着走：新服务器从空开始。',
        ...
        advancedFeatureFooter: '高级功能。切换会用你已有的密钥登录新服务器，无需退出登录。',
        reauthenticating: '正在登录新服务器…',
        reauthFailed: '无法用你的密钥登录该服务器。没有任何改动。',
        services: '服务',
    ...
    sessionInfo: {
        ...
        remoteControl: 'Claude Code 远程控制',
        remoteControlOn: '开 — 可以从手机和 claude.ai/code 操作这个会话',
        remoteControlOff: '关 — 回复只留在终端里',
        remoteControlPending: '正在等待提示符空出来…',
        remoteControlUnknown: '终端还没报告',
        remoteControlFooter: '读取窗格真正的状态，所以在终端里敲的 /remote-control 这里也会显示。',
        remoteControlUnknownFooter: '在会话报告远程控制状态之前无法切换。',
        remoteControlTurnOn: '开启',
        remoteControlAtRiskTitle: '换账号后安静下来的会话',
        remoteControlAtRiskFooter: 'Claude Code 每台机器只把远程控制绑定到一个账号，所以移动这个会话切断了它们。',
        viewMachine: '查看设备',
        ...
        updateCliInstructions: '请运行 npm install -g drover@latest',
        deleteSession: '删除会话',
    ...
    agentInput: {
        ...
        channels: {
            ...
            phoneHaptics: '手机触感',
            phoneHapticsSubtitle: '提示，以及指下的轻触反馈',
            phoneHapticsTitle: '这台手机',
            phoneHapticsFooter: '默认关闭。手表照常震动。',
        },
        codexPermissionMode: {
            title: 'CODEX 权限模式',
            default: 'CLI 设置',
    ...
    settingsVoice: {
        ...
        speaking: {
            ...
            automatic: '自动',
            automaticSubtitle: ({ name }: { name: string }) => `已安装的最佳声音：${name}`,
            automaticNone: '此语言没有已安装的声音',
            preview: '试听',
            previewSentence: '测试通过，两个文件有改动，其他没有变化。',
            qualityPremium: '高级',
            qualityEnhanced: '增强',
            qualityDefault: '紧凑',
            personal: '个人声音',
            deliveryTitle: '朗读方式',
            deliveryFooter: '语速和音调适用于所有声音。已经写完的回复无论多长都会读完。回复仍在生成时，若未读的音频超过阈值，才会说“跳到最新”并从最新的句子继续；新的回复总会放弃上一条。',
            rate: '语速',
            catchUpRate: '落后时的语速',
            pitch: '音调',
            backlog: '落后超过此时长时加速',
            ...
            noNaturalVoice: ({ language }: { language: string }) => `${language} 没有安装增强或高级声音，回复会用紧凑型声音朗读。请在设置中前往 辅助功能 > 朗读内容 > 声音 下载一个。`,
            openSettings: '打开设置',
        },
        cues: {
            title: '音频提示',
            footer: '无需看屏幕的声音。有任务在跑时是缓慢的低沉心跳，有事等你处理时是更快更亮的脉冲，什么都没发生时保持安静。语音永远优先：提示音不会盖在正在朗读的句子上。',
            on: '音频提示',
            onSubtitle: '心跳和一次性提示音。',
            heartbeat: '心跳',
            ...
            waitingEvery: '等待脉冲间隔',
            titlesTitle: '朗读标题',
            titlesFooter: '朗读工具调用、终端调用和新启动智能体的一行标题，语速更快、音调更高，不会听起来像 Claude 在说话。连续的工具调用有数量上限，智能体则始终朗读。',
            speakTitles: '朗读标题',
            agentTitles: '智能体标题',
            agentTitlesSubtitle: '从不省略。',
            toolTitles: '工具和终端标题',
            titlesPerRun: '每串工具调用朗读的标题数',
            tableTitle: '声音',
            tableFooter: '点按一行将其静音，点播放按钮试听。',
            play: '播放',
        },
    },
    ...
    machine: {
        ...
        offlineHelp: '• 确保您的计算机在线\n• 运行 `drover daemon status` 进行诊断\n• 您是否在运行最新的 CLI 版本？请使用 `npm install -g drover@latest` 升级',
        daemon: '守护进程',
        status: '状态',
        stopDaemon: '停止守护进程',
    ...
    subagent: {
        ...
        failed: '失败',
        quiet: ({ duration }: { duration: string }) => `已静默 ${duration}`,
        loading: '正在加载记录…',
        unavailable: '无法获取记录',
        result: '结果',
        title: '代理',
    ...
    imageUpload: {
        ...
        notSupportedFragment: '此代理无法处理图片',
        // The Add context sheet the plus opens (DROVE-128).
        addContextTitle: '添加上下文',
        sourceCamera: '相机',
        sourcePhotos: '照片',
        sourceFiles: '文件',
        cameraPermissionTitle: '相机访问权限',
        cameraPermissionMessage: '允许访问相机以将照片附加到消息中。',
    },
