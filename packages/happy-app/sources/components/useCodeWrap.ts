import * as React from 'react';
import { storage, useSetting } from '@/sync/storage';
import { sync } from '@/sync/sync';
import { CodeWrapKind, isCodeWrapOn, toggleCodeWrap } from '@/sync/settings';
import { hapticsSelection } from './haptics';

/**
 * Whether monospace text of one kind soft-wraps, and the toggle (DROVE-95).
 *
 * One synced setting, so a double-tap on any terminal card flips every
 * terminal card, on this phone and the next one that syncs. The toggle reads
 * the live store rather than the rendered value: two quick double-taps must
 * not both compute from the same stale snapshot and cancel out.
 */
export function useCodeWrap(kind: CodeWrapKind): [boolean, () => void] {
    const codeWrap = useSetting('codeWrap');
    const on = isCodeWrapOn({ codeWrap }, kind);
    const toggle = React.useCallback(() => {
        hapticsSelection();
        sync.applySettings(toggleCodeWrap({ codeWrap: storage.getState().settings.codeWrap }, kind));
    }, [kind]);
    return [on, toggle];
}
