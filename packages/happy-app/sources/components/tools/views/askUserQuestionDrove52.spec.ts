...
describe('the answer given in tmux', () => {
    it('is read off the tool result, which is the only place it exists', () => {
        ...
        expect(answersFromResult(result, questionCards(input))).toEqual({
    ...
    it('splits a multi-select back apart on the separator every surface writes', () => {
        ...
        expect(answersFromResult({ answers: { [QUESTION]: 'One, Two' } }, cards)).toEqual({
            'question-0': ['One', 'Two'],
        });
        expect(answersFromResult({ answers: { [QUESTION]: 'One, Two' } }, [{ question: QUESTION }])).toEqual({
            'question-0': ['One, Two'],
        });
    });
    ...
    it('says nothing rather than an empty map when the result has no answers', () => {
        ...
        expect(answersFromResult(undefined, questionCards(input))).toBeUndefined();
        expect(answersFromResult('Your questions have been answered', questionCards(input))).toBeUndefined();
        expect(answersFromResult({ answers: {} }, questionCards(input))).toBeUndefined();
        expect(answersFromResult({ answers: { 'a different question': 'x' } }, questionCards(input)))
            .toBeUndefined();
    });
});
...
describe('what the settled card shows', () => {
    ...
    it('marks the option the tmux answer chose, and adds nothing else', () => {
        const answers = answersFromResult(result, questionCards(input))!['question-0'];
