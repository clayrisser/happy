 ...
 *
 * DROVE-167 wired `session.todos` to `TodoWrite` and shipped three surfaces on
 * top of it. Clay then said, a fourth time, that he still could not see his
...
/** The list in the one shape every surface downstream already reads. */
export function claudeTaskListToTodos(list: ClaudeTaskList): TodoItem[] {
    const todos: TodoItem[] = [];
    for (const [id, task] of list) {
        todos.push({ id, content: task.content, status: task.status });
    }
    return todos;
}
